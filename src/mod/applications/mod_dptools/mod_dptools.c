...
{
	switch_channel_t *channel = switch_core_session_get_channel(session);
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "Waiting for answer\n");
	while (1) {
		if (switch_channel_test_flag(channel, CF_ANSWERED)) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "Channel answered!\n");
			break;
		}
		switch_ivr_sleep(session, 100, SWITCH_TRUE, NULL);
		if (switch_channel_get_state(channel) >= CS_HANGUP) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "Channel hangup!\n");
			break;
		}
	}
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "Done waiting for answer\n");
}
